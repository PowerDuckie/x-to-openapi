...
Production-grade, extensible TypeScript framework that converts source formats (currently curl commands) into valid **OpenAPI 3.2** documents. Built for CI pipelines, API documentation generation, and reverse-engineering HTTP traffic.

---

## Features

- **curl command parsing** — single commands, multi-command batches, browser "Copy all as cURL" output, Windows `curl.exe`, PowerShell prompts, backslash and caret continuations
- **All body types** — JSON, XML, form-urlencoded, multipart/form-data (text + file fields), text, binary
...
- **Security inference** — Bearer, Basic, and API key (header / query / cookie) detection with proper OpenAPI security schemes
...
- **Extensible adapter architecture** — implement `SourceAdapter` to add Postman collections, HAR files, HTTPie commands, etc.
...
`source` can be an adapter id (`"curl"`) or `"auto"` to select via `canHandle()`.
...
### `validateOpenApi32(document)`

Validates a document against the OpenAPI 3.2 schema. Returns `{ valid, diagnostics }`.
...
| `builder.test.ts`      | 50+   | Document structure, methods, query/header/cookie params, all body types, security, useServerBasePath, collisions |
